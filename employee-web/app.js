...
botonHuella.addEventListener("click", async () => {
  ...
  try {
    ...
    try {
      ...
      if (err.code === 1) {
        textoEstado.textContent =
          "Necesitamos tu ubicación para fichar. Activá el permiso de ubicación para este sitio y volvé a intentar.";
      } else if (err.code === 3) {
        textoEstado.textContent =
          "No pudimos obtener tu ubicación a tiempo. Probá de nuevo con mejor señal de GPS.";
      } else {
        textoEstado.textContent =
          "Tu dispositivo no pudo compartir la ubicación. Probá de nuevo.";
      }
      return;
    }
    ...
    if (distancia > RADIO_PERMITIDO_M) {
      textoEstado.textContent = `Estás a ${Math.round(
        distancia
      )} m del lugar de trabajo. Tenés que estar a menos de ${RADIO_PERMITIDO_M} m para fichar.`;
      return;
    }
    ...
    if (estadoActual === "adentro") {
      await ficharSalida(user.uid, ubicacion);
    } else {
      await ficharEntrada(user, ubicacion);
    }
  } catch (err) {
    console.error(err);
    ...
    botonHuella.removeAttribute("disabled");
    // Si no cambiamos el mensaje por un error, el listener de estado
    // ya se encarga de mostrar el texto correcto tras fichar.
...
async function ficharEntrada(user, ubicacion) {
...
async function ficharSalida(uid, ubicacion) {
  const fichajesRef = collection(db, "fichajes");
  const q = query(
    fichajesRef,
    where("uid", "==", uid),
