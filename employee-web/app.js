...
// Confirma si un horario (semanal recurrente, quincenal, o de fechas
// sueltas) aplica en una fecha puntual del calendario.
function claseAplicaEnFecha(h, fecha) {
  const fechaStr = fechaAStringLocal(fecha);

  if (h.tipo === "fechas") {
    return Array.isArray(h.fechas) && h.fechas.includes(fechaStr);
  }

  // tipo "semanal" (o sin tipo, para horarios viejos cargados antes de este cambio)
  if (fecha.getDay() !== h.diaSemana) return false;

  const frecuencia = h.frecuenciaSemanas || 1;
  if (frecuencia <= 1) return true;
  if (!h.fechaBase) return true;

  const base = new Date(h.fechaBase + "T00:00:00");
  const diffDias = Math.round((fecha - base) / (1000 * 60 * 60 * 24));
  if (diffDias < 0) return false;
  const diffSemanas = Math.floor(diffDias / 7);
  return diffSemanas % frecuencia === 0;
}

function fechaAStringLocal(d) {
...
function horariosDelDia(fecha) {
  return misHorarios
    .filter((h) => claseAplicaEnFecha(h, fecha))
...
function escucharReemplazosAbiertos() {
  ...
  unsubscribeReemplazosAbiertos = onSnapshot(q, (snap) => {
      ...
      .map((docSnap) => {
        ...
        return `
          <div class="tarjeta-solicitud">
            ...
            ${
              esPropio
                ? `<br /><span class="estado-solicitud estado-pendiente">Tu pedido</span> <button class="boton-texto-chico" data-id="${docSnap.id}" data-accion="cancelar-reemplazo">Cancelar</button>`
    ...
    listaReemplazosAbiertos.querySelectorAll('[data-accion="cancelar-reemplazo"]').forEach((boton) => {
      boton.addEventListener("click", () => cancelarReemplazo(boton.dataset.id));
    });
  });
}
...
async function cancelarReemplazo(id) {
  const ok = confirm("¿Cancelar este pedido de reemplazo?");
  if (!ok) return;
  try {
    await updateDoc(doc(db, "reemplazos", id), { estado: "cancelado" });
  } catch (err) {
    console.error(err);
...
// --- Acceso directo a WhatsApp con la admin, con el teléfono
// configurado desde el panel (colección configuracion/contacto) ---
const linkWhatsapp = document.getElementById("link-whatsapp");

onSnapshot(doc(db, "configuracion", "contacto"), (snap) => {
  if (!linkWhatsapp) return;
  const telefono = snap.exists() ? snap.data().telefonoWhatsapp : null;
  if (telefono) {
    linkWhatsapp.href = `https://wa.me/${telefono}`;
    linkWhatsapp.classList.remove("oculto");
  } else {
