...
export default function Reemplazos() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'horarios'), (snap) => {
      setHorarios(snap.docs.map((d) => d.data()));
    });
    return unsub;
  }, []);

  // Lista de profes únicas (nombre + email), para el desplegable de "quién cubre".
  const profesDisponibles = useMemo(() => {
    const mapa = new Map();
    horarios.forEach((h) => {
      if (h.email && h.nombre && !mapa.has(h.email)) mapa.set(h.email, h.nombre);
    });
